...
export default function NewRecruitPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data, error: fetchError } = await (supabase as any)
        .from("recruitments")
        .select("title, description, target_body_part, event_date, deadline_at, location")
        .eq("id", copyId)
        .maybeSingle();
      if (cancelled) return;
      if (fetchError || !data) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const deadlineAt = new Date(deadlineDateTime).toISOString();
              ...
              <Popover open={deadlineOpen} onOpenChange={setDeadlineOpen}>
                <PopoverTrigger asChild>
                  ...
                  >
                    <CalendarIcon className="h-4 w-4 shrink-0 text-gold/70" />
                    {deadlineDate
                      ? new Date(deadlineDate + "T12:00:00").toLocaleDateString("ja-JP", {
                          year: "numeric",
                          month: "long",
                          day: "numeric",
                        })
