LINEの認証が期限切れか無効です。もう一度「LINEでログイン」からやり直してください。

## LINE ログインボタンが押せない・404 が出る

- **ボタンが押せない**: ブラウザの開発者ツール（F12）→ Network タブで、どの URL が 404 を返しているか確認。`/api/auth/line` なら API ルートのデプロイを確認。
- **favicon.ico など別リソースの 404**: ログインには影響しない。無視してよい。
- **本番で /api/auth/line が 404**: ビルド成功・再デプロイを確認。
- **ローカル確認**: `curl -I http://localhost:3000/api/auth/line` で 200 なら API は正常。
