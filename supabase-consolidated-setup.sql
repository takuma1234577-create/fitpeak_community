-- ============================================
-- 統合セットアップ SQL（1〜10 を実行順・冪等にまとめたもの）
-- 既存プロジェクトのスキーマに合わせてあります（reports.type / recruitment_participants PK 等）
-- 二重実行してもエラーにならないように DROP POLICY IF EXISTS を使用しています。
-- ============================================
...
-- ※ アプリは reports の target_id(text), type(text) を参照しています
-- ---------------------------------------------------------------
...
-- 通報: target_id は text（ユーザーID/募集ID/グループID 等を文字列で保存）
...
-- ※ アプリは (recruitment_id, user_id) を PK で使用しています
-- ---------------------------------------------------------------
...
-- ---------------------------------------------------------------
-- ６. 会話・参加者・メッセージテーブル
-- ※ 既に schema.sql で作成済みの場合はスキップされます (CREATE TABLE IF NOT EXISTS)
-- ポリシーは schema.sql の RLS をそのまま利用してください。ここでは追加のみ。
-- ---------------------------------------------------------------
-- (スキップ: schema.sql で定義済み。テスト用の「誰でも読み書き」に変える場合は別ファイルを参照)

-- ---------------------------------------------------------------
-- ７. Facebook 用カラム + follows テーブル + avatars バケット
-- ※ アプリは facebook_url を参照しています（facebook_link ではなく）
-- ---------------------------------------------------------------
ALTER TABLE public.profiles
...
-- ============================================
-- ここまでで 1〜10 の内容を反映済みです。
-- 全テーブルを消して最初から作り直す場合は supabase-RESET-FULL.sql を参照してください。
-- ============================================
